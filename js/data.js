export const emotions = [
  {
    id: 'cansada',
    label: 'Cansada / Agobiada',
    description: 'Para cuando el cuerpo o la mente no dan más.',
    theme: {
      background: 'linear-gradient(135deg, #e0c3fc 0%, #8ec5fc 100%)', // Soft Lavender to Blue
      textColor: '#2c3e50',
      accentColor: '#6c5ce7'
    },
    verses: [
      {
        text: "Venid a mí todos los que estáis trabajados y cargados, y yo os haré descansar.",
        reference: "Mateo 11:28"
      },
      {
        text: "El da esfuerzo al cansado, y multiplica las fuerzas al que no tiene ningunas.",
        reference: "Isaías 40:29"
      },
      {
        text: "En paz me acostaré, y asimismo dormiré; Porque solo tú, Jehová, me haces vivir confiado.",
        reference: "Salmos 4:8"
      },
      {
        text: "Porque he satisfecho al alma cansada, y he saciado a toda alma entristecida.",
        reference: "Jeremías 31:25"
      },
      {
        text: "Y él dijo: Mi presencia irá contigo, y te daré descanso.",
        reference: "Éxodo 33:14"
      },
      {
        text: "Jehová es mi pastor; nada me faltará. En lugares de delicados pastos me hará descansar; Junto a aguas de reposo me pastoreará. Confortará mi alma.",
        reference: "Salmos 23:1-3"
      },
      {
        text: "Pero los que esperan a Jehová tendrán nuevas fuerzas; levantarán alas como las águilas; correrán, y no se cansarán; caminarán, y no se fatigarán.",
        reference: "Isaías 40:31"
      },
      {
        text: "Desde el cabo de la tierra clamaré a ti, cuando mi corazón desmayare. Llévame a la roca que es más alta que yo.",
        reference: "Salmos 61:2"
      },
      {
        text: "Por demás es que os levantéis de madrugada, y vayáis tarde a reposar, y que comáis pan de dolores; pues que a su amado dará Dios el sueño.",
        reference: "Salmos 127:2"
      },
      {
        text: "Con sus plumas te cubrirá, y debajo de sus alas estarás seguro; escudo y adarga es su verdad.",
        reference: "Salmos 91:4"
      },
      {
        text: "En lugares de delicados pastos me hará descansar; Junto a aguas de reposo me pastoreará.",
        reference: "Salmos 23:2"
      }
    ]
  },
  {
    id: 'triste',
    label: 'Triste',
    description: 'Para el dolor profundo o melancolía.',
    theme: {
      background: 'linear-gradient(120deg, #a1c4fd 0%, #c2e9fb 100%)', // Soft Blue
      textColor: '#2c3e50',
      accentColor: '#2980b9'
    },
    verses: [
      {
        text: "Cercano está Jehová a los quebrantados de corazón; y salva a los contritos de espíritu.",
        reference: "Salmos 34:18"
      },
      {
        text: "El sana a los quebrantados de corazón, y venda sus heridas.",
        reference: "Salmos 147:3"
      },
      {
        text: "Bienaventurados los que lloran, porque ellos recibirán consolación.",
        reference: "Mateo 5:4"
      },
      {
        text: "Porque un momento será su ira, pero su favor dura toda la vida. Por la noche durará el lloro, y a la mañana vendrá la alegría.",
        reference: "Salmos 30:5"
      },
      {
        text: "También vosotros ahora tenéis tristeza; pero os volveré a ver, y se gozará vuestro corazón, y nadie os quitará vuestro gozo.",
        reference: "Juan 16:22"
      },
      {
        text: "Bendito sea el Dios y Padre de nuestro Señor Jesucristo, Padre de misericordias y Dios de toda consolación, el cual nos consuela en todas nuestras tribulaciones...",
        reference: "2 Corintios 1:3-4"
      },
      {
        text: "¿Por qué te abates, oh alma mía, y por qué te turbas dentro de mí? Espera en Dios; porque aún he de alabarle, Salvación mía y Dios mío.",
        reference: "Salmos 42:11"
      },
      {
        text: "Enjugará Dios toda lágrima de los ojos de ellos; y ya no habrá muerte, ni habrá más llanto, ni clamor, ni dolor; porque las primeras cosas pasaron.",
        reference: "Apocalipsis 21:4"
      },
      {
        text: "Mis huidas tú has contado; pon mis lágrimas en tu redoma; ¿no están ellas en tu libro?",
        reference: "Salmos 56:8"
      },
      {
        text: "Desde el cabo de la tierra clamaré a ti, cuando mi corazón desmayare. Llévame a la roca que es más alta que yo.",
        reference: "Salmos 61:2"
      },
      {
        text: "Tú lo has visto; porque miras el trabajo y la vejación, para dar la recompensa con tu mano; a ti se acoge el desvalido; tú eres el amparo del huérfano.",
        reference: "Salmos 10:14"
      }
    ]
  },
  {
    id: 'desanimada',
    label: 'Desanimada',
    description: 'Cuando ha perdido la motivación o la visión.',
    theme: {
      background: 'linear-gradient(to top, #cfd9df 0%, #e2ebf0 100%)', // Soft Grey/Silver
      textColor: '#34495e',
      accentColor: '#7f8c8d'
    },
    verses: [
      {
        text: "Mira que te mando que te esfuerces y seas valiente; no temas ni desmayes, porque Jehová tu Dios estará contigo en dondequiera que vayas.",
        reference: "Josué 1:9"
      },
      {
        text: "No temas, porque yo estoy contigo; no desmayes, porque yo soy tu Dios que te esfuerzo; siempre te ayudaré, siempre te sustentaré con la diestra de mi justicia.",
        reference: "Isaías 41:10"
      },
      {
        text: "No nos cansemos, pues, de hacer bien; porque a su tiempo segaremos, si no desmayamos.",
        reference: "Gálatas 6:9"
      },
      {
        text: "Hubiera yo desmayado, si no creyese que veré la bondad de Jehová en la tierra de los vivientes. Aguarda a Jehová; Esfuérzate, y aliéntese tu corazón; Sí, espera a Jehová.",
        reference: "Salmos 27:13-14"
      },
      {
        text: "Pero esforzaos vosotros, y no desfallezcan vuestras manos, pues hay recompensa para vuestra obra.",
        reference: "2 Crónicas 15:7"
      },
      {
        text: "El día que clamé, me respondiste; Me fortaleciste con vigor en mi alma.",
        reference: "Salmos 138:3"
      },
      {
        text: "Estando persuadido de esto, que el que comenzó en vosotros la buena obra, la perfeccionará hasta el día de Jesucristo.",
        reference: "Filipenses 1:6"
      },
      {
        text: "Considerad a aquel que sufrió tal contradicción de pecadores contra sí mismo, para que vuestro ánimo no se canse hasta desmayar.",
        reference: "Hebreos 12:3"
      },
      {
        text: "Mas la senda de los justos es como la luz de la aurora, que va en aumento hasta que el día es perfecto.",
        reference: "Proverbios 4:18"
      },
      {
        text: "Jehová el Señor es mi fortaleza, el cual hace mis pies como de ciervas, y en mis alturas me hace andar.",
        reference: "Habacuc 3:19"
      },
      {
        text: "Alzaré mis ojos a los montes; ¿De dónde vendrá mi socorro? Mi socorro viene de Jehová, que hizo los cielos y la tierra.",
        reference: "Salmos 121:1-2"
      }
    ]
  },
  {
    id: 'ansiosa',
    label: 'Ansiosa / Preocupada',
    description: 'Una de las más comunes hoy en día, necesita paz mental.',
    theme: {
      background: 'linear-gradient(120deg, #f6d365 0%, #fda085 100%)', // Soft Peach/Orange
      textColor: '#5d4037',
      accentColor: '#e67e22'
    },
    verses: [
      {
        text: "Por nada estéis afanosos, sino sean conocidas vuestras peticiones delante de Dios en toda oración y ruego, con acción de gracias. Y la paz de Dios, que sobrepasa todo entendimiento, guardará vuestros corazones y vuestros pensamientos en Cristo Jesús.",
        reference: "Filipenses 4:6-7"
      },
      {
        text: "Echando toda vuestra ansiedad sobre él, porque él tiene cuidado de vosotros.",
        reference: "1 Pedro 5:7"
      },
      {
        text: "Tú guardarás en completa paz a aquel cuyo pensamiento en ti persevera; porque en ti ha confiado.",
        reference: "Isaías 26:3"
      },
      {
        text: "Así que, no os afanéis por el día de mañana, porque el día de mañana traerá su afán. Basta a cada día su propio mal.",
        reference: "Mateo 6:34"
      },
      {
        text: "En la multitud de mis pensamientos dentro de mí, Tus consolaciones alegraban mi alma.",
        reference: "Salmos 94:19"
      },
      {
        text: "La paz os dejo, mi paz os doy; yo no os la doy como el mundo la da. No se turbe vuestro corazón, ni tenga miedo.",
        reference: "Juan 14:27"
      },
      {
        text: "En el día que temo, Yo en ti confío.",
        reference: "Salmos 56:3"
      },
      {
        text: "La congoja en el corazón del hombre lo abate; Mas la buena palabra lo alegra.",
        reference: "Proverbios 12:25"
      },
      {
        text: "Mirad las aves del cielo, que no siembran, ni siegan, ni recogen en graneros; y vuestro Padre celestial las alimenta. ¿No valéis vosotros mucho más que ellas?",
        reference: "Mateo 6:26"
      },
      {
        text: "En verdad que he comportado y he acallado mi alma como un niño destetado de su madre; como un niño destetado está mi alma.",
        reference: "Salmos 131:2"
      }
    ]
  },
  {
    id: 'temerosa',
    label: 'Temerosa / Con Miedo',
    description: 'Ante el futuro o situaciones específicas.',
    theme: {
      background: 'linear-gradient(to top, #accbee 0%, #e7f0fd 100%)', // Very Soft Blue/Grey
      textColor: '#2c3e50',
      accentColor: '#3498db'
    },
    verses: [
      {
        text: "Porque no nos ha dado Dios espíritu de cobardía, sino de poder, de amor y de dominio propio.",
        reference: "2 Timoteo 1:7"
      },
      {
        text: "Jehová es mi luz y mi salvación; ¿de quién temeré? Jehová es la fortaleza de mi vida; ¿de quién he de atemorizarme?",
        reference: "Salmos 27:1"
      },
      {
        text: "Ahora, así dice Jehová, Creador tuyo, oh Jacob, y Formador tuyo, oh Israel: No temas, porque yo te redimí; te puse nombre, mío eres tú.",
        reference: "Isaías 43:1"
      },
      {
        text: "Busqué a Jehová, y él me oyó, y me libró de todos mis temores.",
        reference: "Salmos 34:4"
      },
      {
        text: "Aunque ande en valle de sombra de muerte, No temeré mal alguno, porque tú estarás conmigo; Tu vara y tu cayado me infundirán aliento.",
        reference: "Salmos 23:4"
      },
      {
        text: "Con sus plumas te cubrirá, Y debajo de sus alas estarás seguro; Escudo y adarga es su verdad.",
        reference: "Salmos 91:4"
      },
      {
        text: "Jehová está conmigo; no temeré lo que me pueda hacer el hombre.",
        reference: "Salmos 118:6"
      },
      {
        text: "En el amor no hay temor, sino que el perfecto amor echa fuera el temor...",
        reference: "1 Juan 4:18"
      },
      {
        text: "En Dios he confiado; no temeré; ¿Qué puede hacerme el hombre?",
        reference: "Salmos 56:11"
      },
      {
        text: "Porque yo Jehová soy tu Dios, quien te sostiene de tu mano derecha, y te dice: No temas, yo te ayudo.",
        reference: "Isaías 41:13"
      },
      {
        text: "Porque él me esconderá en su tabernáculo en el día del mal; me ocultará en lo reservado de su morada; sobre una roca me pondrá en alto.",
        reference: "Salmos 27:5"
      }
    ]
  },
  {
    id: 'sola',
    label: 'Sola',
    description: 'Para recordarle la omnipresencia de Dios y tu amor.',
    theme: {
      background: 'linear-gradient(120deg, #e0c3fc 0%, #8ec5fc 100%)', // Reusing soft lavender/blue
      textColor: '#2c3e50',
      accentColor: '#9b59b6'
    },
    verses: [
      {
        text: "Y he aquí yo estoy con vosotros todos los días, hasta el fin del mundo.",
        reference: "Mateo 28:20"
      },
      {
        text: "Aunque mi padre y mi madre me dejaran, con todo, Jehová me recogerá.",
        reference: "Salmos 27:10"
      },
      {
        text: "No te desampararé, ni te dejaré.",
        reference: "Hebreos 13:5"
      },
      {
        text: "Cercano está Jehová a todos los que le invocan, A todos los que le invocan de veras.",
        reference: "Salmos 145:18"
      },
      {
        text: "¿A dónde me iré de tu Espíritu? ¿Y a dónde huiré de tu presencia? Si subiere a los cielos, allí estás tú; Y si en el Seol hiciere mi estrado, he aquí, allí tú estás.",
        reference: "Salmos 139:7-8"
      }
    ]
  },
  {
    id: 'enojada',
    label: 'Enojada / Irritada',
    description: 'Para traer calma y dominio propio.',
    theme: {
      background: 'linear-gradient(to top, #ff9a9e 0%, #fecfef 99%, #fecfef 100%)', // Soft Pink/Red
      textColor: '#c0392b',
      accentColor: '#e74c3c'
    },
    verses: [
      {
        text: "Por esto, mis amados hermanos, todo hombre sea pronto para oír, tardo para hablar, tardo para airarse; porque la ira del hombre no obra la justicia de Dios.",
        reference: "Santiago 1:19-20"
      },
      {
        text: "Airaos, pero no pequéis; no se ponga el sol sobre vuestro enojo, ni deis lugar al diablo.",
        reference: "Efesios 4:26-27"
      },
      {
        text: "La blanda respuesta quita la ira; Mas la palabra áspera hace subir el furor.",
        reference: "Proverbios 15:1"
      },
      {
        text: "Mejor es el que tarda en airarse que el fuerte; Y el que se enseñorea de su espíritu, que el que toma una ciudad.",
        reference: "Proverbios 16:32"
      },
      {
        text: "Deja la ira, y desecha el enojo; No te excites en manera alguna a hacer lo malo.",
        reference: "Salmos 37:8"
      },
      {
        text: "La cordura del hombre detiene su furor, Y su honra es pasar por alto la ofensa.",
        reference: "Proverbios 19:11"
      },
      {
        text: "Soportándoos unos a otros, y perdonándoos unos a otros si alguno tuviere queja contra otro. De la manera que Cristo os perdonó, así también hacedlo vosotros.",
        reference: "Colosenses 3:13"
      },
      {
        text: "No te apresures en tu espíritu a enojarte; porque el enojo reposa en el seno de los necios.",
        reference: "Eclesiastés 7:9"
      },
      {
        text: "...y tras el terremoto un fuego, pero Jehová no estaba en el fuego. Y tras el fuego un silbo apacible y delicado.",
        reference: "1 Reyes 19:11-12"
      },
      {
        text: "Temblad, y no pequéis; meditad en vuestro corazón estando en vuestra cama, y callad.",
        reference: "Salmos 4:4"
      }
    ]
  },
  {
    id: 'insegura',
    label: 'Insegura / Con dudas',
    description: 'Cuando duda de su valor o capacidad.',
    theme: {
      background: 'linear-gradient(120deg, #d4fc79 0%, #96e6a1 100%)', // Soft Green
      textColor: '#27ae60',
      accentColor: '#2ecc71'
    },
    verses: [
      {
        text: "Te alabaré; porque formidables, maravillosas son tus obras; Estoy maravillado, Y mi alma lo sabe muy bien.",
        reference: "Salmos 139:14"
      },
      {
        text: "Porque a mis ojos fuiste de gran estima, fuiste honorable, y yo te amé.",
        reference: "Isaías 43:4"
      },
      {
        text: "Porque somos hechura suya, creados en Cristo Jesús para buenas obras, las cuales Dios preparó de antemano para que anduviésemos en ellas.",
        reference: "Efesios 2:10"
      },
      {
        text: "Y me ha dicho: Bástate mi gracia; porque mi poder se perfecciona en la debilidad.",
        reference: "2 Corintios 12:9"
      },
      {
        text: "Antes que te formase en el vientre te conocí, y antes que nacieses te santifiqué, te di por profeta a las naciones.",
        reference: "Jeremías 1:5"
      },
      {
        text: "¿Qué, pues, diremos a esto? Si Dios es por nosotros, ¿quién contra nosotros?",
        reference: "Romanos 8:31"
      },
      {
        text: "Todo lo puedo en Cristo que me fortalece.",
        reference: "Filipenses 4:13"
      },
      {
        text: "Mas vosotros sois linaje escogido, real sacerdocio, nación santa, pueblo adquirido por Dios...",
        reference: "1 Pedro 2:9"
      },
      {
        text: "Porque tú formaste mis entrañas; Tú me hiciste en el vientre de mi madre.",
        reference: "Salmos 139:13"
      },
      {
        text: "¿No se venden dos pajarillos por un cuarto? Con todo, ni uno de ellos cae a tierra sin vuestro Padre. Pues aun vuestros cabellos están todos contados.",
        reference: "Mateo 10:29-31"
      }
    ]
  },
  {
    id: 'agradecida',
    label: 'Agradecida',
    description: 'Para potenciar la gratitud hacia el Padre.',
    theme: {
      background: 'linear-gradient(to right, #ffecd2 0%, #fcb69f 100%)', // Soft Orange/Pink
      textColor: '#d35400',
      accentColor: '#e67e22'
    },
    verses: [
      {
        text: "Bendice, alma mía, a Jehová, Y bendiga todo mi ser su santo nombre. Bendice, alma mía, a Jehová, Y no olvides ninguno de sus beneficios.",
        reference: "Salmos 103:1-2"
      },
      {
        text: "Dad gracias en todo, porque esta es la voluntad de Dios para con vosotros en Cristo Jesús.",
        reference: "1 Tesalonicenses 5:18"
      },
      {
        text: "Toda buena dádiva y todo don perfecto desciende de lo alto, del Padre de las luces, en el cual no hay mudanza, ni sombra de variación.",
        reference: "Santiago 1:17"
      },
      {
        text: "Este es el día que hizo Jehová; Nos gozaremos y alegraremos en él.",
        reference: "Salmos 118:24"
      },
      {
        text: "Y todo lo que hacéis, sea de palabra o de hecho, hacedlo todo en el nombre del Señor Jesús, dando gracias a Dios Padre por medio de él.",
        reference: "Colosenses 3:17"
      },
      {
        text: "Alabad a Jehová, porque él es bueno; Porque para siempre es su misericordia.",
        reference: "Salmos 107:1"
      },
      {
        text: "Dando siempre gracias por todo al Dios y Padre, en el nombre de nuestro Señor Jesucristo.",
        reference: "Efesios 5:20"
      },
      {
        text: "Jehová es mi fortaleza y mi escudo; En él confió mi corazón, y fui ayudado, Por lo que se gozó mi corazón, Y con mi cántico le alabaré.",
        reference: "Salmos 28:7"
      },
      {
        text: "...Unges mi cabeza con aceite; mi copa está rebosando.",
        reference: "Salmos 23:5"
      },
      {
        text: "Las cuerdas me cayeron en lugares deleitosos, y es hermosa la heredad que me ha tocado.",
        reference: "Salmos 16:6"
      }
    ]
  },
  {
    id: 'gozosa',
    label: 'Gozosa',
    description: 'Más profundo que la alegría; es el estado del espíritu conectado a Dios.',
    theme: {
      background: 'linear-gradient(120deg, #f093fb 0%, #f5576c 100%)', // Vibrant Pink/Purple
      textColor: '#c0392b',
      accentColor: '#e91e63'
    },
    verses: [
      {
        text: "...no os entristezcáis, porque el gozo de Jehová es vuestra fuerza.",
        reference: "Nehemías 8:10"
      },
      {
        text: "Me mostrarás la senda de la vida; En tu presencia hay plenitud de gozo; Delicias a tu diestra para siempre.",
        reference: "Salmos 16:11"
      },
      {
        text: "Aunque la higuera no florezca... Con todo, yo me alegraré en Jehová, Y me gozaré en el Dios de mi salvación.",
        reference: "Habacuc 3:17-18"
      },
      {
        text: "Estas cosas os he hablado, para que mi gozo esté en vosotros, y vuestro gozo sea cumplido.",
        reference: "Juan 15:11"
      },
      {
        text: "Tú diste alegría a mi corazón Mayor que la de ellos cuando abundaba su grano y su mosto.",
        reference: "Salmos 4:7"
      },
      {
        text: "En gran manera me gozaré en Jehová, mi alma se alegrará en mi Dios; porque me vistió con vestiduras de salvación, me rodeó de manto de justicia...",
        reference: "Isaías 61:10"
      },
      {
        text: "...a quien amáis sin haberle visto, en quien creyendo, aunque ahora no lo veáis, os alegráis con gozo inefable y glorioso.",
        reference: "1 Pedro 1:8"
      },
      {
        text: "Has cambiado mi lamento en baile; Desataste mi cilicio, y me ceñiste de alegría.",
        reference: "Salmos 30:11"
      },
      {
        text: "Entonces nuestra boca se llenará de risa, y nuestra lengua de alabanza...",
        reference: "Salmos 126:2"
      },
      {
        text: "Porque con alegría saldréis, y con paz seréis vueltos; los montes y los collados levantarán canción delante de vosotros, y todos los árboles del campo darán palmadas de aplauso.",
        reference: "Isaías 55:12"
      }
    ]
  },
  {
    id: 'esperanzada',
    label: 'Esperanzada',
    description: 'Mirando al futuro con fe.',
    theme: {
      background: 'linear-gradient(to top, #a8edea 0%, #fed6e3 100%)', // Soft Teal/Pink
      textColor: '#16a085',
      accentColor: '#1abc9c'
    },
    verses: [
      {
        text: "Porque yo sé los pensamientos que tengo acerca de vosotros, dice Jehová, pensamientos de paz, y no de mal, para daros el fin que esperáis.",
        reference: "Jeremías 29:11"
      },
      {
        text: "La cual tenemos como segura y firme ancla del alma, y que penetra hasta dentro del velo.",
        reference: "Hebreos 6:19"
      },
      {
        text: "Y el Dios de esperanza os llene de todo gozo y paz en el creer, para que abundéis en esperanza por el poder del Espíritu Santo.",
        reference: "Romanos 15:13"
      },
      {
        text: "Esto recapacitaré en mi corazón, por lo tanto esperaré... Mi porción es Jehová, dijo mi alma; por tanto, en él esperaré.",
        reference: "Lamentaciones 3:21, 24"
      },
      {
        text: "Y ahora, Señor, ¿qué esperaré? Mi esperanza está en ti.",
        reference: "Salmos 39:7"
      },
      {
        text: "Y la esperanza no avergüenza; porque el amor de Dios ha sido derramado en nuestros corazones por el Espíritu Santo que nos fue dado.",
        reference: "Romanos 5:5"
      },
      {
        text: "Porque tú, oh Señor Jehová, eres mi esperanza, Seguridad mía desde mi juventud.",
        reference: "Salmos 71:5"
      },
      {
        text: "Esperé yo a Jehová, esperó mi alma; En su palabra he esperado.",
        reference: "Salmos 130:5"
      },
      {
        text: "Bendito el varón que confía en Jehová... Porque será como el árbol plantado junto a las aguas, que junto a la corriente echará sus raíces...",
        reference: "Jeremías 17:7-8"
      },
      {
        text: "Por la misericordia de Jehová no hemos sido consumidos, porque nunca decayeron sus misericordias. Nuevas son cada mañana; grande es tu fidelidad.",
        reference: "Lamentaciones 3:22-23"
      }
    ]
  },
  {
    id: 'amada',
    label: 'Amada',
    description: 'Para que recuerde su identidad como Hija de Dios.',
    theme: {
      background: 'linear-gradient(to top, #ff9a9e 0%, #fecfef 99%, #fecfef 100%)', // Soft Pink
      textColor: '#e91e63',
      accentColor: '#d81b60'
    },
    verses: [
      {
        text: "Jehová se manifestó a mí hace ya mucho tiempo, diciendo: Con amor eterno te he amado; por tanto, te prolongué mi misericordia.",
        reference: "Jeremías 31:3"
      },
      {
        text: "Mas Dios muestra su amor para con nosotros, en que siendo aún pecadores, Cristo murió por nosotros.",
        reference: "Romanos 5:8"
      },
      {
        text: "Mirad cuál amor nos ha dado el Padre, para que seamos llamados hijos de Dios...",
        reference: "1 Juan 3:1"
      },
      {
        text: "Jehová está en medio de ti, poderoso, él salvará; se gozará sobre ti con alegría, callará de amor, se regocijará sobre ti con cánticos.",
        reference: "Sofonías 3:17"
      },
      {
        text: "Toda tú eres hermosa, amiga mía, Y en ti no hay mancha.",
        reference: "Cantares 4:7"
      },
      {
        text: "He aquí que en las palmas de las manos te tengo esculpida; delante de mí están siempre tus muros.",
        reference: "Isaías 49:16"
      },
      {
        text: "Por lo cual estoy seguro de que ni la muerte, ni la vida, ni ángeles, ni principados, ni potestades, ni lo presente, ni lo por venir, ni lo alto, ni lo profundo, ni ninguna otra cosa creada nos podrá separar del amor de Dios, que es en Cristo Jesús Señor nuestro.",
        reference: "Romanos 8:38-39"
      },
      {
        text: "Como el Padre me ha amado, así también yo os he amado; permaneced en mi amor.",
        reference: "Juan 15:9"
      },
      {
        text: "Mi amado habló, y me dijo: Levántate, oh amiga mía, hermosa mía, y vente. Porque he aquí ha pasado el invierno, se ha mudado, la lluvia se fue.",
        reference: "Cantares 2:10-11"
      },
      {
        text: "Porque a mis ojos fuiste de gran estima, fuiste honorable, y yo te amé...",
        reference: "Isaías 43:4"
      }
    ]
  }
];

export const welcomeMessage = {
  title: "Hola, mi amor",
  body: "He creado este pequeño refugio para ti. Quiero que sepas que en cada momento, Dios tiene una palabra para tu corazón. No importa cómo te sientas hoy, Él está contigo, y yo también.",
  buttonText: "¿Cómo te sientes hoy?"
};
